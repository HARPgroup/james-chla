...
C**********************************************************************C
C
      IF (JSVSFP.EQ.0) GO TO 100
      OPEN(1,FILE='vsfp1.out',STATUS='UNKNOWN')
      CLOSE(1,STATUS='DELETE')
      OPEN(1,FILE='vsfp1.out',STATUS='UNKNOWN')
C      WRITE(1,101)
C      WRITE(1,102)
      CLOSE(1)      
      OPEN(1,FILE='vsfp2.out',STATUS='UNKNOWN')
      CLOSE(1,STATUS='DELETE')
      JSVSFP=0
C
C**********************************************************************C
...
C
  ...
  200         K=K+1
        ...
        END DO
C
        OPEN(1,FILE='vsfp1.out',ACCESS='APPEND',STATUS='UNKNOWN')
     ...
     $                          (SNDOT(MD,NX),NX=1,NSND)
        ...
        CLOSE(1)
C
        OPEN(1,FILE='vsfp2.out',ACCESS='APPEND',STATUS='UNKNOWN')
        WRITE(1,103)TIMVSFP(ML),NTVSFP(ML),I,J,HP(L)
        WRITE(1,204)
        DO K=KC,1,-1
...
C**********************************************************************C
C
  101 FORMAT(1X,'INSTANTANEOUS VERTICAL SCALAR FIELD PROFILES')
  102 FORMAT(/)
  103 FORMAT(1X,'TIME = ',F12.4,' N = ',I8,'  I,J = ',2I4,
     $          '  H = ',F10.2)
  104 FORMAT(1X,'DEP BEL SURF     SAL,TEM,DYE,SFL',/)
  114 FORMAT(1X,'DEP BEL SURF     SED(N=1,NSED),SND(N=1,NSND)',/)
  124 FORMAT(1X,'DEP BEL SURF     TOX(N= 1, 6)',/)
  134 FORMAT(1X,'DEP BEL SURF     TOX(N= 7,12)',/)
  144 FORMAT(1X,'DEP BEL SURF     TOX(N=13,18)',/)
  154 FORMAT(1X,'DEP BEL SURF     TOX(N=19,24)',/)
  204 FORMAT(1X,'LAY     DBS     HAB       SAL,TEM,DYE,SFL',/)
  214 FORMAT(1X,'LAY     DBS     HAB       ',
     $          'SED(N=1,NSED),SND(N=1,NSND)',/)
  224 FORMAT(1X,'LAY     DBS     HAB       TOX(N= 1, 6)',/)
  234 FORMAT(1X,'LAY     DBS     HAB       TOX(N= 7,12)',/)
  244 FORMAT(1X,'LAY     DBS     HAB       TOX(N=13,18)',/)
  254 FORMAT(1X,'LAY     DBS     HAB       TOX(N=19,24)',/)
