...
C
    5 CONTINUE
      NTSPTCP=NTSPTC+1
      OPEN(80,FILE='lrpd.log',ACCESS='APPEND',STATUS='UNKNOWN')
C
      DO M=1,MLRPDRT
...
C
      ...
      DO KLR=1,KC
      DO NLR=1,NLRPD
      XMLRPD(NLR,KLR,M)=RTMP*(XMLRPD(NLR,KLR,M)-0.5*XLRPD(NLR,KLR,M))
      YMLRPD(NLR,KLR,M)=RTMP*(YMLRPD(NLR,KLR,M)-0.5*YLRPD(NLR,KLR,M))
      ZMLRPD(NLR,KLR,M)=RTMP*(ZMLRPD(NLR,KLR,M)-0.5*ZLRPD(NLR,KLR,M))
      END DO 
      END DO
      END DO
C
...
C
        ...
        DO NLR=1,NLRPD
          IF (NUTMP(NLR,KLR).NE.0) THEN
            RTMP=1./FLOAT(NUTMP(NLR,KLR))
            UPLRPD(NLR,KLR)=RTMP*UPLRPD(NLR,KLR)
            VPLRPD(NLR,KLR)=RTMP*VPLRPD(NLR,KLR)
            WPLRPD(NLR,KLR)=RTMP*WPLRPD(NLR,KLR)
            NUTMP(NLR,KLR)=1
           END IF
        END DO
        END DO
...
C
      ...
      DO NLR=1,NLRPD
        IF (NUMTMP(NLR,KLR).NE.0) THEN
          RTMP=1./FLOAT(NUMTMP(NLR,KLR))
          UMPLRPD(NLR,KLR)=RTMP*UMPLRPD(NLR,KLR)
          VMPLRPD(NLR,KLR)=RTMP*VMPLRPD(NLR,KLR)
          WMPLRPD(NLR,KLR)=RTMP*WMPLRPD(NLR,KLR)
          NUMTMP(NLR,KLR)=1
        END IF
      END DO
      END DO
