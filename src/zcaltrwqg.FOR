...
C**********************************************************************C
C 
      SUBROUTINE CALTRWQG (M,NW,CON,CON1)
C
...
!     i.e.  N-2, N,  N+2, ....
!     
C     A General transport sub.  J.Shen
C
C**********************************************************************C
C
      INCLUDE 'efdc.par'
...
!      CALL CALFQC (ISTL,MVAR,M,CON,CON1)
      IF(IRELSOP.EQ.2) THEN
       IF(abs(TIME-RELTIME).LT.1.0e-6) NREL=1
       IF(NREL.GE.1 .and. NREL.LE.NSTEPREL) THEN
         LL=LIJ(IRDYE,JRDYE)
         IF(KRDYE.GE.1) THEN
            FQC(LL,KRDYE)=RELMASS
         ELSE
          DO KK=1,KC 
           FQC(LL,KK)=RELMASS
          ENDDO
      ...
      NCSTMP=NCSERQ(NS,M)
       DO K=1,KC
       FQC(L,K)=FQC(L,K)
     $         +MAX(QSS(K,NS),0.)*1.0 !CQS(K,NS,M)
     $         +MIN(QSS(K,NS),0.)*1.0 !CON1(L,K) 
     $         +MAX(QSERT(K,NQSTMP),0.)*1.0 !CSERT(K,NCSTMP,M)
     $         +MIN(QSERT(K,NQSTMP),0.)*1.0 !CON1(L,K)
       END DO
      END DO      
      ENDIF
C**********************************************************************C
...
C
     ...
     $    +CSERTWQ(K,NSID,NW)
       IF(IRELSOP.GT.0)CBT=0  !J.S. force BC for dye release Need change if relase from mouth 1/9/2015
       NMNLO=N-NWQLOS(LL,K,NW)
       IF (NMNLO.GE.NTSCRS(LL)) THEN
       IF(IREBCTP.EQ.0) THEN
        CON(L,K)=CBT
        ELSE
